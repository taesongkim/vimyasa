...
export function ItemRow({
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, x: -40 }}
      transition={{ duration: 0.15, type: 'spring', stiffness: 400, damping: 35 }}
    ...
    >
      ...
      {editing ? (
        ...
        <span
          className={`flex-1 text-sm truncate ${
            isDone ? 'line-through text-[var(--color-text-dim)]' : ''
          }`}
        >
          {item.text}
        </span>
      ...
      {hovered && !editing && (
        <div className="flex items-center gap-1 shrink-0">
          <button
            className="no-drag p-1 rounded text-[var(--color-text-dim)] hover:text-[var(--color-text)] hover:bg-[var(--color-surface)] transition-colors"
            onClick={(e) => {
              e.stopPropagation()
          ...
          <button
            className="no-drag p-1 rounded text-[var(--color-text-dim)] hover:text-[var(--color-accent)] hover:bg-[var(--color-surface)] transition-colors"
            onClick={(e) => {
              e.stopPropagation()
          ...
          <button
            className="no-drag p-1 rounded text-[var(--color-text-dim)] hover:text-[var(--color-amber)] hover:bg-[var(--color-surface)] transition-colors"
            onClick={(e) => {
              e.stopPropagation()
              archiveItem(item.id)
            }}
      ...
      {hovered && !editing && (
        <div
          className="no-drag cursor-grab active:cursor-grabbing text-[var(--color-text-dim)] hover:text-[var(--color-text-muted)]"
          {...attributes}
          {...listeners}
        >
