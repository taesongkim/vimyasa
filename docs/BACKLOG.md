...
Live release: **v0.1.13**. Planned next sequence:
...
| ~~**v0.1.11**~~ | ~~Auto-update actually works + list flick gone~~ | ✅ Shipped as a tight hotfix — only the two P1/P2 bug fixes, all deferred v0.1.10 items pushed to v0.1.12. (1) Replaced `autoUpdater.quitAndInstall()` with a direct `child_process.spawn` of ShipIt in `src/main/updater.ts`, bypassing the silently-failing Squirrel.framework spawn on macOS 26. Includes fallback to `quitAndInstall()` if the direct spawn ever throws. (2) Removed `useUpwardFlip` and renamed `data-flip-id` → `data-item-row-id` (preserved the carry-mode dimming CSS + arriving-item scroll lookup). Rows now snap into place on upward shifts — matches the app's motion vocabulary. Step 4 (verify auto-update path pre-publish) was exercised for the first time on this release — Justin's v0.1.10 install successfully auto-updated to v0.1.11 via the actual flow, confirming the fix end-to-end. Post-ship: coordination outreach to every ≤v0.1.10 tester with a manual DMG install link (they can't receive v0.1.11 via auto-update because the broken code is what they're running). |
| ~~**v0.1.12**~~ | ~~Small polish pass~~ | ✅ Shipped — four small things: (1) "Create new list" as first item in the list-window three-dot menu; reuses tray's addList path. (2) Shortcuts overview window now draggable — added `drag-region` class to its header strip. (3) Tab now suppressed during item edit (existing item + new-item draft) with a transient "Press Enter to save" hint using the shared confirmation overlay pattern. Backdrop bumped from 0.2 → 0.55 alpha + content dim added to DraftItemRow for parity with ItemRow so the hint reads cleanly. (4) Tab-on-single-list documented as intentional no-op (guard was already in `cycleToNextList`). Deferred v0.1.10/v0.1.11 items pushed to v0.1.13 to keep this pass tight. |
| ~~**v0.1.13**~~ | ~~Auto-update actually fixed + download progress UX~~ | ✅ Shipped — real fix for the macOS 26 auto-update silent-install bug. v0.1.11's direct-spawn attempt turned out to be broken at design level: it bypassed `autoUpdater.quitAndInstall()`, which is what writes the ShipIt state plist that ShipIt reads to know what to install where. v0.1.13's fix: register a `will-quit` listener that spawns ShipIt ourselves, then call `quitAndInstall()` normally — Squirrel writes the state plist + tries (and fails on macOS 26) its own spawn, we then spawn ShipIt in will-quit with the state plist Squirrel just wrote. Also: download-progress window UX — window stays visible through the download with a live progress bar (via electron-updater's `download-progress` event) and transitions smoothly into the install prompt on completion. Deferred items pushed to v0.1.14. |
| **v0.1.14** | Deferred items (fifth push) | Backup / restore user data (P1). Focus-state visual cue (P2). Mystery flicker on first entry-form launch (P2). Onboarding dim z-order (P3). Scrollbar tracking lag (P3). Auto-updater EPIPE defensive wrap (P3). Coordination: auto-update integrity audit. Continuously pushed since v0.1.10 while we've been shipping tight hotfixes. Since v0.1.13's auto-update fix should genuinely work now, v0.1.14 can actually be the release where the deferred items land — no more emergency hotfix cycles blocking them. |
...
- **Design decision (Justin, 2026-07-24):** **Option A — remove `useUpwardFlip` entirely.** Snap-only behavior (no upward animation on shifts) tested cleanly in dev with the hook disabled; matches Justin's motion ethos ("snappiness above almost everything"). The visual-confirmation-of-action value the FLIP was providing isn't worth the maintenance surface + the flick it produced.
- **Scope (removal):**
  1. Delete the call at [`ListWindow.tsx:106`](../src/renderer/src/components/ListWindow/ListWindow.tsx) — `useUpwardFlip(itemsContainerRef, '[data-flip-id]')`.
  2. Delete the import at the top of ListWindow.tsx.
  3. Delete the file `src/renderer/src/hooks/useUpwardFlip.ts`.
  4. Remove `data-flip-id={item.id}` from `ItemRow.tsx` (line ~433) plus its explanatory comment (lines ~429-432 in v0.1.10).
  5. Remove the CSS rule `.list-carrying [data-flip-id]:not(.item-row-carrying)` in `globals.css` (line ~1251) — it was scoped to the hook's tagged elements; without them, the selector is dead.
  6. Grep for any other stale references (should be minimal).
- **Verify:** in dev, run through: archive an item in middle of a list → items snap up cleanly (no flick), no residual animation debris; carry mode + move up then back down → siblings snap without flicking; nothing else regresses (edit-mode row shrinking still looks fine as a snap; new item add at top still fine).
- **Reconsider path (if snap ever feels wrong later):** if a future release brings back the "visual confirmation" desire, use one of the not-chosen paths from the 2026-07-24 diagnosis: (B1) bump FLIP duration to 100-150ms so invert+transition read as one clean slide, or (B2) replace the manual reflow with `requestAnimationFrame` for reliable atomicity, or (B3) apply the invert transform inline in the render return (React commits both DOM + transform in same paint). Note in this entry if any of those get revisited.
