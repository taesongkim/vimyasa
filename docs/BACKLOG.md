...
| **v0.1.9** | Update-pipeline UX + Magic Colors light-mode | Themes lane: Magic Colors light-mode calibration (controls-panel iteration with Justin's eyes). Features lane: promote About surface to production + render current version's release notes; user-facing update tray entries (Check for updates / View update details). Three items, all small polish on shipped v0.1.8 features. |
